...
# You cannot directly import xts objects. However, you can import e.g.
# spreadsheets as a zoo object and convert it to xts.
read.csv.zoo(read = "x.csv") %>% 
  as.xts()


# To export an xts object, use write.zoo. The only required arguments are an xts
# object and the filenamet. Unless not otherwise possible, I strongly recommend
# exporting to csv using the code below.
...
# For times, you have to put "T" before the values. For example, all observations
# from 8:30 to 15:00:
x["T08:30/T15:00"]


# The xts equivalent to head() and tail() are first() and last(). You must
# supply an xts object and an indication of what should be extracted. The latter
# can either be an integer for the number of observations, or a string
# containing a number and the time range (e.g. weeks). If you use a string, time
# ranges like "week" or "month" will return the respective values like a
# calender. If a given week only has three days as entries, it will only return
# these three entries, not seven days.
first(x, 3)
last(x, 11)
first(x, "3 weeks")
last(x, "11 days")
...
# function directly to an xts object. The first argument is an xts object, the
# second indicates if NAs should be removed, the third indicates if the last
# non-missing value (forward replacing) or the next (backward replacing) should
# be used to fill missing values, the fourth argument sets an upper limit to how
# many adjacent NAs this should be applied.
...
# You can compute Open-High-Low-Close aggregations by using the to.period()
